...
export default {
  ...
  setup(props, { emit }) {
    ...
    const visibleToasts = computed(() => {
      const max = props.content.maxToasts || 5
      return toasts.value.slice(0, max)
    })

    const getPositionClasses = () => {
      const position = props.content.position || 'bottom-right'
    ...
    const addToast = (toastData) => {
      ...
      if (props.content.position?.startsWith('bottom')) {
    ...
    watch(() => props.content.toastQueue, (newQueue) => {
